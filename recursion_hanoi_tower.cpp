...
// 汉诺塔问题：　　如下图所示，从左到右有A、B、C三根柱子，其中A柱子上面有从小叠到大的n个圆盘，
// 现要求将A柱子上的圆盘移到C柱子上去，期间只有一个原则：一次只能移到一个盘子且大盘子不能在小盘子上面，求移动的步骤和移动的次数

// n - 1 个由A放在B上，n由A放在C上，n-1个由B放在C上

// hanoid(n) = hanoid(n-1) + 以上过程
...
void hanoid(int n, char a, char b, char c) 
{
    ...
    static int ith = 1;
    hanoid( n - 1,  a,  c,  b);
    printf("%dth step, move %s from %c to %c\n",  ith++, to_string(n).c_str(), a, c);
    hanoid( n - 1,  b,  a,  c);
}
